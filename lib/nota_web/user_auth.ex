defmodule NotaWeb.UserAuth do
  ...
  @remember_me_cookie "_nota_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      NotaWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      NotaWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule NotaWeb.PageLive do
        use NotaWeb, :live_view

        on_mount {NotaWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{NotaWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_god, _params, session, socket) do
    socket = mount_current_scope(socket, session)

    case socket.assigns.current_scope do
      %{user: %Accounts.User{is_god: true}} ->
        {:cont, socket}
